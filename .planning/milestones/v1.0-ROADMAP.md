...
Plans:
- [x] 02-01-PLAN.md — Static type lookup table (pokemonTypes.js) and PokemonCard component
- [x] 02-02-PLAN.md — Wire Results.jsx to use PokemonCard; production build verification
...
Plans:
- [x] 03-01-PLAN.md — Full-page type gradient and NavBar type tint (DETAIL-01)
- [x] 03-02-PLAN.md — Stat bar threshold colors and polish (DETAIL-02, DETAIL-03)
- [x] 03-03-PLAN.md — Move table headers, dark rows, and pill badge harmonization (DETAIL-04)
...
Plans:
- [x] 04-01-PLAN.md — Dex number watermark on grid cards (POLISH-01)
- [x] 04-02-PLAN.md — Shimmer skeleton loading cards (POLISH-02)
- [x] 04-03-PLAN.md — Prev/Next Pokémon navigation on detail page (POLISH-03)

## Milestone Summary

**Key Decisions:**
- Dark-always approach (no `dark:` class variants, no `prefers-color-scheme`) — entire app is unconditionally dark ✓ Good
- Static `pokemonTypes.js` lookup over API fetching — zero additional API calls, instant type-color rendering ✓ Good
- Inline styles for dynamic gradients (`var(--color-{type})`) avoids Tailwind JIT purge ✓ Good
- CSS custom property on `:root` (`--active-type-color`) for NavBar type signal — zero prop drilling ✓ Good
- `@source inline()` registry for dynamic Tailwind class patterns — required for Tailwind 4 ✓ Good
- Zinc palette for surfaces (zinc-900/800/700/600 hierarchy) ✓ Good

**Issues Resolved:**
- Silent production build bug: stat bar widths broken via Tailwind JIT string interpolation — fixed with inline styles
- LearnSet gray palette gap found after Phase 1 — closed in plan 01-03
